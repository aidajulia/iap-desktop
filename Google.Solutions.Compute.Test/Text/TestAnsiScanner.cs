...
namespace Google.Solutions.Compute.Test.Text
{
    [TestFixture]
    public class TestAnsiScanner : FixtureBase
    {
        ...
        public async Task WhenInputIsNull_ThenReadReturnsNull()
        {
            ...
            var reader = new AnsiScanner(new EnumerationReader<string>(input));
            Assert.IsNull(await reader.ReadAsync());
            Assert.IsNull(await reader.ReadAsync());
        }
        ...
        {
            var esc = AnsiScanner.Escape.ToString();
            var input = new string[] {
